...
// ----------------------------
// Create or replace view for available products (with category name)
$create_view_sql = "
...
// ----------------------------
// Handle search (only product name, case-insensitive)
$search_name = isset($_GET['search_name']) ? mysqli_real_escape_string($conn, $_GET['search_name']) : '';
$sql_products = "SELECT * FROM AvailableProducts WHERE 1=1";

if ($search_name != "") {
    $sql_products .= " AND TRIM(name) COLLATE utf8mb4_general_ci LIKE '%$search_name%'";
}
...
<body>
    ...
    <!-- 🔍 Search Form (only product name) -->
    <form method="get">
        <input type="text" name="search_name" placeholder="Search by product name"
            value="<?php echo htmlspecialchars($search_name); ?>">
        <button type="submit">Search</button>
        <button type="button" onclick="window.location.href='create_order.php'">Reset</button>
